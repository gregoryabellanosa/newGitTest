...
public class JPADemo {
    ...
    public static void processInput(JPADemo demo, String userInput) {
        ...
        switch (userInput.toLowerCase()) {
            ...
            case "remove team":
                ...
                break;
            case "set captain":
                System.out.println("Set a Captain");
                System.out.print("Team name:");
                teamName = USER_INPUT.nextLine();
                /*First name and last name of the captain
                 that we are designating
                 to the specified team above.
                 */
                System.out.print("First Name: ");
                String fName = USER_INPUT.nextLine();
                System.out.print("Last Name: ");
                String lName = USER_INPUT.nextLine();
                demo.setTeamCaptain(teamName, fName, lName);
                break;
            case "view captain":
                System.out.println("View the Captain:");
                System.out.print("Team name:");
                teamName = USER_INPUT.nextLine();
                System.out.print(demo.getCaptain(teamName));
                break;
    ...
    /**
     * Locates the player and then executes a query to add this player as the team's captain
     * @param teamName The name of the team who we are setting/updating the team's captain
     * @param firstName The first name of the player we are searching to be team captain
     * @param lastName  The last name of the player we are searching to be the team's captain.
     */
    private void setTeamCaptain(String teamName, String firstName, String lastName)
    {

        TypedQuery<Player> retrievePlayerQuery = ENTITY_MANAGER.createNamedQuery(Player.GET_PLAYER_LIST, Player.class);
        retrievePlayerQuery.setParameter("name", teamName);
        List<Player> players = retrievePlayerQuery.getResultList();
        boolean found = false;
        Player captain = null;
        ...
        if (!found)
        {
            System.out.print("Either team or player specified not found, please try again");
        }
        else
        {
            TypedQuery<Team> updateTeamCaptain = ENTITY_MANAGER.createNamedQuery(Team.SET_CAPTAIN_BY_TEAM, Team.class);
            updateTeamCaptain.setParameter("cplayer", captain);
            updateTeamCaptain.setParameter("name", teamName);


            ENTITY_MANAGER.getTransaction().begin();
            updateTeamCaptain.executeUpdate();
            THE_LOGGER.fine("CHECK DB... you'll see captain player has been set");
            ENTITY_MANAGER.getTransaction().commit();
    ...
    private Player getCaptain(String teamName)
    {

        TypedQuery<Player> retrieveTeamQuery = ENTITY_MANAGER.createNamedQuery(Team.FIND_CAPTAIN, Player.class);
        retrieveTeamQuery.setParameter("name", teamName);
        List<Player> teams = retrieveTeamQuery.getResultList();
        Player captain = teams.get(0);
        return captain;
     ...
     * Menu choices
     */
    private static final String MENU_CHOICES[] = {"reload", "find", "roster", "remove player", "remove team", "set captain", "view captain", "quit"};
    ...
    "Set a captain for the team",
    "View a captain for the team",
    "Quit this program"
    };
